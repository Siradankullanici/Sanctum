...
use wdk_sys::{ntddk::{CmCallbackGetKeyObjectIDEx, CmCallbackReleaseKeyObjectIDEx, CmRegisterCallbackEx, CmUnRegisterCallback, RtlInitUnicodeString}, LARGE_INTEGER, NTSTATUS, REG_DELETE_KEY_INFORMATION, REG_NOTIFY_CLASS, STATUS_ACCESS_DENIED, STATUS_SUCCESS, UNICODE_STRING, _REG_NOTIFY_CLASS::RegNtPreDeleteKey};
...
pub fn enable_registry_monitoring(driver_object: *mut c_void) -> Result<(), i32> {
    ...
    unsafe { RtlInitUnicodeString(&mut altitude_unicode, altitude_string.as_ptr()); }
    let results = unsafe {
    ...
    if let Err(e) = Grt::register_fast_mutex("registry_monitor", registration_cookie) {
        println!("[sanctum] [-] Failed to store registry monitor in GRT. {:?}", e);
        return Err(12345678);
    }
...
pub unsafe fn unregister_registry_monitor() {
    let cookie:Result<&FastMutex<LARGE_INTEGER>, wdk_mutex::errors::GrtError>  = Grt::get_fast_mutex("registry_monitor");
    if cookie.is_err() {
        return;
    }
